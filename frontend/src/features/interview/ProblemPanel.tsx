...
export const ProblemPanel = ({ problem }: { problem: CandidateProblem }) => (
  <div className="space-y-4 rounded-md border border-white/10 bg-surface p-5">
    ...
    <p className="whitespace-pre-wrap text-sm leading-7 text-graphite">{problem.prompt}</p>
      ...
      <div>
        ...
        <div className="mt-2 space-y-2">
          {problem.examples.map((example) => (
            <div
              key={example.input}
              className="rounded-md border border-white/5 bg-slatewash p-3 font-mono text-xs text-ink"
            >
